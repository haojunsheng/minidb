...
// set default store diretory，$HOME/MiniDBData/
// instantiate object MiniDBAPI
Interpreter::Interpreter() : sql_type_(-1) {
  string p = string(getenv("HOME")) + "/MiniDBData/";
  api = new MiniDBAPI(p);
}

// destruct,eg: ClassName::~ClassName
Interpreter::~Interpreter() { delete api; }

// store sql statement in sql_vector_
...
void Interpreter::Run() {
  try {
    ...
    case 31: {
      //SQLCreateTable instantiate
      // SQLCreateTable includes sql_type,tb_name,and attrs
      // attrs is a array,each element is a Attribute object, includes attr_name,data_type,length,attr_type
      SQLCreateTable *st = new SQLCreateTable(sql_vector_);
      // actually,init Table object,and recode catalog
      api->CreateTable(*st);
     // finally, delete SQLCreateTable Object
      delete st;
    } break;
    case 32: {
    ...
    case 70: {
      //SQLInsert instantiate
      // SQLInsert includes sql_type,tb_name,and values
      // values is a array,each element includes data_type,value
...
void Interpreter::ExecSQL(string statement) {
  ...
  FormatSQL();
  // according to sql_vector_[0] set sql_type_
  TellSQLType();
