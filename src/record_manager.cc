...
// SQLInsert has all data
// file store: Data is stored as a single piece of information inside a folder
// block store: Each block of data is given a unique identifier,
// which allows a storage system to place the smaller pieces of data wherever is most convenient
void RecordManager::Insert(SQLInsert &st) {
  ...
  // consider datatype is index,we need construct index
  // TODO: I will think it in future when create index
  if (pk_index != -1) {
  ...
  // we use block to store data,not file
  char *content;
  ...
  while (ub != -1) {
    ...
    // BufferManger.WriteBlock(BlockInfo)
    hdl_->WriteBlock(bp);
