...
// a uniform class which stores values in SQL statements;
class TKey {
private:
  int key_type_;
  char *key_;
...
public:
  ...
  bool operator<(const TKey t1) {
    ...
    case 1:// 1 means double
      return *(float *)key_ < *(float *)t1.key_;
    case 2: // 2 means string
...
// base class for the following SQL statement classes storing statements;
class SQL {
protected:
  int sql_type_;
